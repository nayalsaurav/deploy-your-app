...
function parseHSL(hslStr: string): { h: number; s: number; l: number } {
  ...
  const sStr = match?.[2];
  const lStr = match?.[3];
  if (!hStr || !sStr || !lStr) return { h: 40, s: 80, l: 80 };
  return { h: parseFloat(hStr), s: parseFloat(sStr), l: parseFloat(lStr) };
}
...
function buildMeshGradients(colors: string[]): string[] {
  ...
  for (let i = 0; i < 7; i++) {
    const mapIndex = COLOR_MAP[i] ?? 0;
    const c = colors[Math.min(mapIndex, colors.length - 1)];
